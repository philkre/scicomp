...
using .Helpers.WaveEquations: propagate_psi, propagate_psi_leapfrog, energy_shifted_from_solution, wave_equation, wave_equation_inb, wave_equation_vec, wave_equation_dist, wave_equation_simd, wave_equation_avx, wave_equation_dist_avx
...
using .Helpers.PlotWave: get_wave_plots, plot_wave_multi, plot_euler_leapfrog_energy
...
function main(; do_bench::Bool=false, do_gif::Bool=false, plot_output_dir::String="plots")
    if do_bench
    ...
    @time "Ran Euler method simulation" begin
        ...
        plot_wave_multi(solution_1_euler, x, "Euler, IC 1", tp_indices; output=joinpath(plot_output_dir, "ex_1_wave_multi_euler_ic1.png"))
        plot_wave_multi(solution_2_euler, x, "Euler, IC 2", tp_indices; output=joinpath(plot_output_dir, "ex_1_wave_multi_euler_ic2.png"))
        plot_wave_multi(solution_3_euler, x, "Euler, IC 3", tp_indices; output=joinpath(plot_output_dir, "ex_1_wave_multi_euler_ic3.png"))
    end

    if do_gif
        @info "Creating animation for Euler method..."
        @time "Created Euler method animation" begin
            distributed_gif(plots, joinpath(plot_output_dir, "ex_1_anim_euler.gif"), do_palette=true)
        end
    end
    ...
    if do_gif
        ...
        @time "Created Leapfrog method animation" begin
            distributed_gif(plots_leapfrog, joinpath(plot_output_dir, "ex_1_anim_leapfrog.gif"), do_palette=true)
        end
    end
    ...
    plot_euler_leapfrog_energy(tvals, energy_euler_shifted, energy_leapfrog_shifted; output=joinpath(plot_output_dir, "ex_1_energy_euler_vs_leapfrog.png"))
end
