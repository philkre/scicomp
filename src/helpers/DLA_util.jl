...
function _write_eta_outputs(output_dir::String, raw_rows::Vector{NamedTuple}, trajectory_rows::Vector{NamedTuple}, dimension_rows::Vector{NamedTuple})
    _write_eta_csv(
        joinpath(output_dir, "eta_sweep_raw.csv"),
        raw_rows,
        "eta,repeat,step,particle_count,gyration_radius",
        row -> "$(row.eta),$(row.repeat),$(row.step),$(row.particle_count),$(row.gyration_radius)",
    )
    _write_eta_csv(
        joinpath(output_dir, "eta_sweep_trajectory_summary.csv"),
        trajectory_rows,
        "eta,step,mean_particle_count,std_particle_count,mean_gyration_radius,std_gyration_radius,n_repeats",
        row -> "$(row.eta),$(row.step),$(row.mean_particle_count),$(row.std_particle_count),$(row.mean_gyration_radius),$(row.std_gyration_radius),$(row.n_repeats)",
    )
    _write_eta_csv(
        joinpath(output_dir, "eta_sweep_dimension_summary.csv"),
        dimension_rows,
        "eta,mean_dimension,std_dimension,n_valid_runs",
        row -> "$(row.eta),$(row.mean_dimension),$(row.std_dimension),$(row.n_valid_runs)",
    )
    return nothing
end
...
function _plot_eta_dimension_experiment(trajectory_rows::Vector{NamedTuple}, dimension_rows::Vector{NamedTuple}; output_path::String)
    eta_values = sort(unique(row.eta for row in trajectory_rows))
    colors = palette(:viridis, max(length(eta_values), 1))
    eta_min = minimum(eta_values)
    eta_max = maximum(eta_values)
    ...
    for (k, eta) in enumerate(eta_values)
        rows = filter(row -> row.eta == eta, trajectory_rows)
    ...
    if !isempty(dimension_rows)
        etas = [row.eta for row in dimension_rows]
    ...
    savefig_auto_folder(p2, root * "_D_vs_eta" * ext)
...
function rerender_eta_dimension_plot_from_csv(;
    ...
    trajectory_rows = _read_eta_summary_csv(
        joinpath(input_dir, "eta_sweep_trajectory_summary.csv"),
        [
            :eta => Float64,
            :step => Int,
    ...
    dimension_rows = _read_eta_summary_csv(
        joinpath(input_dir, "eta_sweep_dimension_summary.csv"),
        [
            :eta => Float64,
    ...
    return _plot_eta_dimension_experiment(trajectory_rows, dimension_rows; output_path=output_path)
end
...
function run_eta_dimension_experiment(;
    ...
    etas = collect(Float64, eta_values)
    isempty(etas) && throw(ArgumentError("eta_values must not be empty"))
    ...
    for (eta_idx, eta) in enumerate(etas)
        if repeats > 0
            @info "Running eta sweep for eta=$eta ($eta_idx/$(length(etas)))"
        end
        ...
        for rep in 1:repeats
            seed!(seed_base + hash((eta, rep, N, backend_resolved, solver)))
            result = run_diffusion_limited_aggregation(
                N,
                L,
                tol,
            ...
            for (step, mask) in zip(result.frame_indices, result.sink_history)
                ...
                push!(Rgs, r_g)
                push!(raw_rows, (eta=eta, repeat=rep, step=step, particle_count=Float64(n_particles), gyration_radius=r_g))
            end
    ...
    save_csv && _write_eta_outputs(output_dir, raw_rows, trajectory_rows, dimension_rows)

    plot_path = joinpath(output_dir, "eta_sweep_M_vs_Rg_and_D.png")
    p = _plot_eta_dimension_experiment(trajectory_rows, dimension_rows; output_path=plot_path)
    @info "Saved eta sweep plot to $plot_path"
